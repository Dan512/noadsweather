...
The 77 static pages under `cities/`, the `/cities/` index page, and `sitemap.xml` are generated by `scripts/build-cities.js` from `index.html`, `scripts/cities.json`, and the city-page i18n keys in `js/i18n.js` + `js/i18n/*.js` (English lives in `js/i18n.js`; every other language has its own file under `js/i18n/`). Re-run the script whenever any of those inputs change:
