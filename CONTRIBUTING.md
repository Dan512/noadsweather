...
5. If you edited `scripts/cities.json`, any city-page i18n keys (`cityPageTitle`, `cityPageSeoBlurb`, `cityPageHideBlurb`, `nearbyCities`), or the `<head>` metadata in `index.html`, re-run `node scripts/build-cities.js` and commit the regenerated `cities/` + `sitemap.xml`
6. Submit a pull request

## Regenerating city pages

The static pages under `cities/`, the `/cities/` index page, and `sitemap.xml` are generated by `scripts/build-cities.js` from `index.html`, `scripts/cities.json`, `scripts/climate-data.json`, and the city-page i18n keys in `js/i18n.js` + `js/i18n/*.js` (English lives in `js/i18n.js`; every other language has its own file under `js/i18n/`). When adding cities, run `node scripts/fetch-climate.js` first to fetch their climate normals. Re-run the build script whenever any of those inputs change:
